...
namespace Millon.TecnicalTest.RealEstate.Application.Common.Validators.Owners
{
    public class OwnerUpdateRequestValidator : AbstractValidator<OwnerUpdateRequest>
    {
        ...
        {
            RuleFor(x => x.Id).NotNull().NotEmpty().WithMessage(OwnerErrors.RequiredId.ErrorMessage).WithErrorCode(OwnerErrors.RequiredId.ErrorCode);
            ...
            RuleFor(x => x.Address).NotNull().NotEmpty().WithMessage(OwnerErrors.RequiredAdress.ErrorMessage).WithErrorCode(OwnerErrors.RequiredAdress.ErrorCode)
                .Length(OwnerModelConstants.OwnerConstants.MinAddressLength, OwnerModelConstants.OwnerConstants.MaxAddressLength).WithMessage(OwnerErrors.RequiredAdress.ErrorMessage).WithErrorCode(OwnerErrors.ValidAdress.ErrorCode);
            ...
            RuleFor(x => x.Photo).NotNull().NotEmpty().WithMessage(OwnerErrors.RequiredPhoto.ErrorMessage).WithErrorCode(OwnerErrors.RequiredPhoto.ErrorCode)
                .Length(OwnerModelConstants.OwnerConstants.MinPhotoLength, OwnerModelConstants.OwnerConstants.MaxPhotoLength).WithMessage(OwnerErrors.RequiredPhoto.ErrorMessage).WithErrorCode(OwnerErrors.ValidPhoto.ErrorCode);
