    ...
    public class UserUpdateRequestValidator : AbstractValidator<UserUpdateRequest>
    {
        public UserUpdateRequestValidator()
        {
            RuleFor(x => x.Id).NotNull().NotEmpty().WithMessage(UserErrors.RequiredId.ErrorMessage).WithErrorCode(UserErrors.RequiredId.ErrorCode)
                .Length(UserModelConstants.Usuario.MinIdLength, UserModelConstants.Usuario.MaxNameLength).WithMessage(UserErrors.ValidId.ErrorMessage).WithErrorCode(UserErrors.ValidId.ErrorCode);
            ...
            RuleFor(x => x.Nombre).NotNull().NotEmpty().WithMessage(UserErrors.RequiredName.ErrorMessage).WithErrorCode(UserErrors.RequiredName.ErrorCode)
                .Length(UserModelConstants.Usuario.MinNameLength, UserModelConstants.Usuario.MaxNameLength).WithMessage(UserErrors.ValidName.ErrorMessage).WithErrorCode(UserErrors.ValidName.ErrorCode);
